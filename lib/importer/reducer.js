...
module.exports = function(filmData, now) {
  now = new Date('2017-10-29T00:00:00.000Z');
  let mostRecentShowtimeStart = moment.tz(now, 'Europe/London').startOf('day');
  const alreadySeenShowtime = new Set();

  let earliestShowtime = mostRecentShowtimeStart;

  const filmDataMap = filmData.reduce(reducer, new Map());

  let latestShowtime = mostRecentShowtimeStart;

  // Check if the DST is different from earliest to latest.
  if(earliestShowtime.isDST() !== latestShowtime.isDST()) {
    // We need to make some adjustments, otherwise
    // the timings will be all off.
    console.log('Detected timezone shift, recalculating showtimes.');
    // TODO implement this!
  }
  ...
  function reducer(map, item) {
    const filmKey = `${item.title}:${item.year || ''}`;
    const showtimeKey = `${filmKey}:${item.showtime.channel}:${item.showtime.startsAt}:${item.showtime.endsAt}`;

    if(!alreadySeenShowtime.has(showtimeKey)) {
      const existingFilm = map.get(filmKey);
